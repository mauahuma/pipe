...
export function GraficoSemana() {
  const today = getCurrentDatelfSemana("-");
    ...
    await axios
      ...
      .then((Response) => {
        ...
        respuesta.map((elemento) => {
          if (auxNombre.indexOf(hourSemana(elemento.created_at)) === -1) {
            auxNombre.push(hourSemana(elemento.created_at));
            auxStock_critico.push(1);
          } else {
            auxStock_critico[auxNombre.indexOf(hourSemana(elemento.created_at))] =
              auxStock_critico[auxNombre.indexOf(hourSemana(elemento.created_at))] +
              1;
          }
        });
...
export function hourSemana(date) {
  let fecha = new Date(date);
  let hour = fecha.getHours();
  return hour;
}
export function getCurrentDatelfSemana(separator = "") {
