...
export function Grafico() {
  const today = getCurrentDatelfMes("-");
    ...
    await axios
      ...
      .then((Response) => {
        ...
        respuesta.map((elemento) => {
          if (auxNombre.indexOf(hourMes(elemento.created_at)) === -1) {
            auxNombre.push(hourMes(elemento.created_at));
            auxStock_critico.push(1);
          } else {
            auxStock_critico[auxNombre.indexOf(hourMes(elemento.created_at))] =
              auxStock_critico[auxNombre.indexOf(hourMes(elemento.created_at))] +
              1;
          }
        });
...
export function hourMes(date) {
  let fecha = new Date(date);
  let hour = fecha.getHours();
  return hour;
}
export function getCurrentDatelfMes(separator = "") {
